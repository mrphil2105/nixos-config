...
local audioMixer = "ghostty --title=wiremix -e wiremix --tab output"
local menu = "walker"
...
for key, workspace in pairs(extraWorkspaceKeys) do
...
hl.bind(mainMod .. " + CONTROL + PRINT", hl.dsp.exec_cmd("hyprshot -m window"))
hl.bind(mainMod .. " + SHIFT + CONTROL + PRINT", hl.dsp.exec_cmd("hyprshot -m region"))
...
hl.bind(mainMod .. " + CONTROL + K", hl.dsp.window.resize({ x = 0, y = -30, relative = true }), { repeating = true })
hl.bind(mainMod .. " + CONTROL + J", hl.dsp.window.resize({ x = 0, y = 30, relative = true }), { repeating = true })
...
hl.bind("switch:off:Lid Switch", hl.dsp.exec_cmd('hyprctl keyword monitor "eDP-1, 1920x1200, 0x0, 1"'), { locked = true })
...
for i, rule in ipairs(workspaceRules) do
  hl.window_rule({
